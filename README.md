...
- MATLAB R2025a (installed at `D:\Matlab R2025a`)
- CasADi 3.8.0 (provided under `MPC_DWA_Fusion/third_party`)
...
manuscript is generated by `paper/build_final_paper_cn.py` from
`paper/EI_paper_final_CN.md`.
...
33/36 and LSTM 36/36; full per-run failure and timing data are in
`results/random_eval/random12_20260906_134750`.
...
The final trained model is kept at `results/lstm_predictor.mat`. Generated
experiment outputs under `results/` are ignored by Git and are staged separately
under `reproducibility_release_v1.0.0/` for the versioned data archive. Replace
the `TODO` archive fields in the release metadata after Zenodo publication.
